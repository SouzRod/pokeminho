...
import { AddFavoritePokemonUseCase, DeleteFavoriteUseCase, GetFavoriteListUseCase, GetPokemonListUseCase } from 'src/application/use-cases';
...
export class PokemonController {
  ...
  async getPokemonList(
    @Query()
    query: GetPokemonListQueryDTO,
  ) {
    return this.getPokemonListUseCase.execute(query.offset?.toString() ?? '0', query.limit?.toString() ?? '10');
  }
