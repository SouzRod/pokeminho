...
describe('AddFavoritePokemonUseCase', () => {
  ...
  const mockApi = { findById: jest.fn() };
  ...
  it('should return existing favorite if found', async () => {
    mockRepo.findById.mockResolvedValue({ id: 1, name: 'pikachu' });
  ...
  it('should fetch from API and create if not found', async () => {
    ...
    mockApi.findById.mockResolvedValue({ id: 2, name: 'bulbasaur' });
    mockRepo.create.mockResolvedValue({ id: 2, name: 'bulbasaur' });
    const result = await useCase.execute('2');
    expect(result.id).toBe(2);
    expect(mockApi.findById).toHaveBeenCalledWith(2);
    expect(mockRepo.create).toHaveBeenCalledWith({ id: 2, name: 'bulbasaur' });
  });
